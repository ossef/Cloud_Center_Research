...
{     
    ...
    {
    printf("Erreur1 :Ligne %d et voisin %d :s5 = %.10le et proba = %.10le et s5 - proba = %.10le \nLa matrice est non monotone :( \n",t,l1 ->voisin,s5,l1->proba,s5 - l1->proba);
    exit(-1);  
    }
...
else
 {
    ...
    if( fabs(s5 - l1->proba)>EPSILON2) //fabs((float)s5 - (float)l1->proba) >EPSILON )
    {
    printf("Erreur2 :Ligne %d et voisin %d s5 = %.15lf et proba = %.15lf et %.15lf \nLa matrice est non monotone :( \n",t,l1->voisin,s5,l1->proba,s5 - l1->proba);
    exit(-1);  
    }
