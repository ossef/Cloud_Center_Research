...
char *argv[];
{
	...
	switch (*++*argv) {
		...
		{++argv;
			...
			Nu1 = atof(*(++argv));
			Nu2 =atof(*(++argv));
			printf("Gen Markov : UP = %d et DOWN = %d et Nu1 = %f et Nu2 = %f \n",UP,DOWN,Nu1,Nu2);
			pf1=fopen(s1,"w");
			pf2=fopen(s2,"w"); 
			break;
		}
	...
	do {
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(F);
#endif
			pro = Probabilite(j, It.voisin,Nu1,Nu2); //proba
			SommeDeProba+=pro;
#if (DEBUG==1)
			printf("Proba : %lf\n", pro);
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(F);
#endif
			pro = Probabilite(j, It.voisin,Nu1,Nu2)/2;
			SommeDeProba+=pro;
#if (DEBUG==1)
				printf("Proba : %lf\n", pro);
        printf("SommeDeProba : %lf\n", SommeDeProba);
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(F);
#endif
			pro1 = Probabilite(j, It.voisin, Nu1,Nu2);
			for (k = 1; k <= NbEvtsPossibles; k++) {
				Equation(F, k, G, R, UP, DOWN);
#if (DEBUG==2)
				PrintEtat(G);
#endif
				pro2 = Probabilite(k, F,Nu1,Nu2);
				pro = pro1 * pro2 /2.0; 
				SommeDeProba+=pro;
#if (DEBUG==1)
				printf("Proba : %lf\n", pro);
