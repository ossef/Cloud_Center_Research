...
{ 
    ...
    /* Calcul de la proba moyenne que le block de serveur est actif (donc au niveau 2) */
    
    double ProbaMoyNiv2 = proba_serveur_actif(et,pi,ReachableStates);
    
    
    pf1=fopen(s5,"a");
    fprintf(pf1,"%12d  ", UP);
    fprintf(pf1,"%12d  ", DOWN);
    fprintf(pf1,"%.12lf  ", perte);
    fprintf(pf1,"%.12lf  ", Consenergie);
    fprintf(pf1,"%.12lf  ", NombreMoyn);
    fprintf(pf1,"%.12lf  ", TimeAttente);
    fprintf(pf1,"%.12lf  ", ProbaMoyNiv2);
