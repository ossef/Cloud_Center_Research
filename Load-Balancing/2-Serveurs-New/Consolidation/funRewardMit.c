...
void energie2(int tab[],double pi[],int taille, char ch[50],double gamma12, int alpha, double Eb, int **M){ // Calcule Energie Nvelle formule en comptant le LB
	...
	FILE *f1=fopen(ch,"a");
		...
		for(i=0; i<taille*2 ; i=i+2){
			...
			if( tab[i] ==1 && tab[i+1]>=1 && tab[i+1]<BufferSize )
			{	
					s3 += Eb*gamma12*pi[(int)i/2];
			}
			if( tab[i+1] ==1 && tab[i]>=1 && tab[i]<BufferSize )
			{
					s4 += Eb*gamma21*pi[(int)i/2];	
			}
			if( tab[i] ==0)
			{
					s5 += Ea*pi[(int)i/2];
		...
		if( Eb == 0 || gamma12 == 0 || gamma21 == 0)
		fprintf(f1,"%lf %lf %lf %lf %lf 0.000000 0.000000 %d %d %d ",Eb,gamma12,s1,s2,s1+s2,gama,beta,alpha);
		else
		fprintf(f1,"%lf %lf %lf %lf %lf %lf %lf %d %d %d ",Eb,gamma12,s1,s2,s1+s2,s3/(Eb*gamma12),s4/(Eb*gamma21),gama,beta,alpha);
		
		s1 = gama*(1-s5) + s1*CPU*beta + (s1*CPU)*(s1*CPU)*alpha + Ea*Lambda1*s5 ;
		s2 = gama*(1-s6) + s2*CPU*beta + (s2*CPU)*(s2*CPU)*alpha + Ea*Lambda2*s6 ;
