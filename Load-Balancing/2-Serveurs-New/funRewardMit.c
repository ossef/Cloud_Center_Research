...
void energie2(int tab[],double pi[],int taille, char ch[50],double gamma12, int alpha, double Eb, int **M){ // Calcule Energie Nvelle formule en comptant le LB
	...
	int i,Inservice,Outservice;
	double  s1 , s2 ,s3 ,s4, s5, s6, st, st1, st2;
	...
	double value = gamma12 / (Lambda1 + Lambda2  + BufferSize*(Mu1 + Mu2) + BufferSize*(gamma12 + gamma21) ) ;
	
	FILE *f1=fopen(ch,"a");

		st = st1 = st2 = s1 = s2 = s3 = s4 = s5 = s6 = 0;
		for(i=0; i<taille*2 ; i=i+2){
			
			if( M[tab[i]][tab[i+1]] == 1 ) //&&  tab[i+1] ==0 && tab[i]>=2 )
			{	
					s3 += Eb*value*pi[(int)i/2];
					s5 += pi[(int)i/2];
			}
			if( M[tab[i]][tab[i+1]] == 2 )// &&  tab[i+1]>=2 && tab[i] ==0 )
			{
					s4 += Eb*value*pi[(int)i/2];	
					s6 += pi[(int)i/2];
			}
		...
		fprintf(f1,"%lf %lf %lf %lf %lf %lf %lf %d %d %d ",Eb,gamma12,s1,s2,s1+s2,s5,s6,gama,beta,alpha);
		s1 = gama + s1*CPU*beta + (st1*CPU*CPU)*alpha ;
		s2 = gama + s2*CPU*beta + (st2*CPU*CPU)*alpha ;
		st = s1+s2+s3+s4;
		fprintf(f1,"%lf %lf %lf %lf",s1,s2,s3+s4,st);
		...
		fprintf(f1," %lf %lf %lf %lf\n",s1,s2,s1+s2,st/(Lambda1+Lambda2-Lambda1*s1-Lambda2*s2));
		fclose(f1);
}
