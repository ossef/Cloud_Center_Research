...
/* --------------------------------------------*/
/* fonction de lecture de la matrice de coup,  */
/* Pour optimiser la migration                 */
/* --------------------------------------------*/
...
void  usage()
{
	printf("usage : GenerMatrix -f filename gamma12\n");
...
char *argv[];
{
	...
	do {
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(It.voisin);
#endif
			Equation(It.voisin, j, F, R, StateReward);
#if (DEBUG==1)
			PrintEtat(F);
#endif
			pro = Probabilite(j, It.voisin, gamma12, StateReward);
			SommeDeProba+=pro;
#if (DEBUG==1)
			printf("Proba : %lf\n", pro);
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(F);
#endif
			pro = Probabilite(j, It.voisin,gamma12,StateReward)/2;
			SommeDeProba+=pro;
#if (DEBUG==1)
				printf("Proba : %lf\n", pro);
        printf("SommeDeProba : %lf\n", SommeDeProba);
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(F);
#endif
			pro1 = Probabilite(j, It.voisin, gamma12,StateReward);
			for (k = 1; k <= NbEvtsPossibles; k++) {
				Equation(F, k, G, R, StateReward);
#if (DEBUG==2)
				PrintEtat(G);
#endif
				pro2 = Probabilite(k, F, gamma12,StateReward);
				pro = pro1 * pro2 /2.0; 
				SommeDeProba+=pro;
#if (DEBUG==1)
				printf("Proba : %lf\n", pro);
