...
char *argv[];
{
	...
	double SommeDeProba;
	short deja;
	double pro, pro1, pro2 , gamma12, Mu1;
	FILE *pf1, *pf2;
	char s1[20];
	char s2[20];
	char s3[20];
	...
	do {
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(F);
#endif
			pro = Probabilite(j, It.voisin, gamma12, Mu1);
			SommeDeProba+=pro;
#if (DEBUG==1)
			printf("Proba : %lf\n", pro);
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(F);
#endif
			pro = Probabilite(j, It.voisin,gamma12, Mu1)/2;
			SommeDeProba+=pro;
#if (DEBUG==1)
				printf("Proba : %lf\n", pro);
        printf("SommeDeProba : %lf\n", SommeDeProba);
		...
		for (j = 1; j <= NbEvtsPossibles; j++) {
			...
			PrintEtat(F);
#endif
			pro1 = Probabilite(j, It.voisin, gamma12, Mu1);
			for (k = 1; k <= NbEvtsPossibles; k++) {
				Equation(F, k, G, R);
#if (DEBUG==2)
				PrintEtat(G);
#endif
				pro2 = Probabilite(k, F, gamma12, Mu1);
				pro = pro1 * pro2 /2.0; 
				SommeDeProba+=pro;
#if (DEBUG==1)
				printf("Proba : %lf\n", pro);
