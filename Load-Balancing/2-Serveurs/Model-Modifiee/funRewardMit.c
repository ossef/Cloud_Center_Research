...
void energie2(int tab[],double pi[],int taille, char ch[50],double gamma12, int alpha){ // Calcule Energie Nvelle formule en comptant le LB
	...
	FILE *f1=fopen(ch,"a");
		...
		fprintf(f1,"%lf %lf %lf %lf %lf %lf %d %d %d %d %d %d  %lf %lf %lf %lf ",gamma12,s1,s2,s1+s2,s5,s6,gama,beta,alpha,gama,beta2,alpha2,s1*CPU*beta,(s1*CPU)*(s1*CPU)*alpha,s2*CPU*beta,(s2*CPU)*(s2*CPU)*alpha);
		s1 = gama + s1*CPU*beta + (s1*CPU)*(s1*CPU)*alpha ;
		s2 = gama + s2*CPU*beta2 + (s2*CPU)*(s2*CPU)*alpha2 ;
