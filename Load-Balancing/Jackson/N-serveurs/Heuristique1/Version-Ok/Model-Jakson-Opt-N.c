...
void Calcul_LB() // glouton
{
	...
	some = 0;
		...
		while( charge_Max() > charge_Min() && Exist_Couple(Icharge_Max(),Icharge_Min()) == -1) //some1 - some > EPSILON && gamma21 > EPSILON )// )  //>= 
		{
				...
				if(imin != imax && R[imax]>R[imin] && Exist_Couple(imax,imin) == -1 )//&& R[imax]>R[imin])
				{
